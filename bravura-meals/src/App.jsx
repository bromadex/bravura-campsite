...
function AppContent() {
  ...
  if (loading) return (
    <div style={{
      ...
      background: `linear-gradient(135deg, ${THEME.sidebarDark} 0%, ${THEME.sidebar} 100%)`,
      color: '#fff', fontSize: '15px', fontFamily: "'Segoe UI', Arial, sans-serif",
  ...
  function getPage() {
    switch (page) {
      ...
      case 'employees':   return ['super_admin','meal_officer','approver'].includes(role)               ? <Employees />      : null
      case 'contractors': return ['super_admin','meal_officer','approver'].includes(role)               ? <Contractors />    : null
    ...
    <Layout page={page} setPage={setPage}>
      {content || (
        <div style={{ textAlign: 'center', padding: '80px 24px', color: '#aaa' }}>
          ...
          <p style={{ fontSize: '14px' }}>You don't have access to this section.</p>
        </div>
      )}
    </Layout>
  )
