...
/** SVG area chart of projected tank level with ±1σ confidence band and dashed threshold line.
 *  curve: [{ date, expected, best, worst }] — best/worst bound the shaded band. */
export function LevelBandChart({ curve, startLevel, capacity, reorderLevel, color, thresholdLabel = 'Reorder threshold' }) {
  const gradId = useId()
  ...
  return (
    <div style={{ position: 'relative' }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', display: 'block', overflow: 'visible' }}>
        ...
        <text x={W - PAD.right} y={y(reorderLevel) - 5} fontSize="10" fill={THEME.warning} fontWeight="600" textAnchor="end">{thresholdLabel}</text>
