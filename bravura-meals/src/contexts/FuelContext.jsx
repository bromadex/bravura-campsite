...
export function FuelProvider({ children }) {
  ...
  async function deleteDelivery(id, reason = 'Cancelled from Tank Deliveries') {
    // F1 (#69): the delivery and its tank transaction are cancelled together (kept as history).
    const { error } = await supabase.rpc('fuel_delivery_void', { p_id: id, p_reason: reason })
    if (error) throw error
    await fetchAll()
  }
