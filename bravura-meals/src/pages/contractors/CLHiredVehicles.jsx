...
const EMPTY_FORM = {
  contractor_id: '', contract_id: '', description: '', registration: '',
  vehicle_type: '', daily_rate: '', hourly_rate: '', km_rate: '',
  driver_included: false, fuel_included: false, assigned_operator: '',
...
export default function CLHiredVehicles() {
  ...
  async function fetchLookups() {
    ...
    const [c1, c2, c3] = await Promise.all([
      supabase.from('contractors').select('id, name').eq('site_id', currentSiteId).eq('is_archived', false).order('name'),
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editId) {
        const { error: err } = await supabase.from('hired_vehicles').update(payload).eq('id', editId)
        if (err) throw err
  ...
  async function handleArchive() {
    if (!confirm('Archive this hired vehicle record?')) return
    try {
      const { error: err } = await supabase.from('hired_vehicles').update({ is_archived: true }).eq('id', editId)
      if (err) throw err
      showToast('Hired vehicle archived', 'green')
      await fetchItems()
      setModalOpen(false)
    } catch (err) {
  ...
  async function handleReturn(id) {
    ...
    try {
      const { error: err } = await supabase.from('hired_vehicles').update({
        status: 'returned', end_date: new Date().toISOString().slice(0, 10),
      }).eq('id', id)
      if (err) throw err
      showToast('Vehicle marked returned', 'green')
      await fetchItems()
    } catch (err) {
      showToast(err.message, 'red')
    }
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ fontSize: '20px', fontWeight: 500, color: THEME.text }}>Hired Vehicles</div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
        <input placeholder="Search description, rego or contractor..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inp, maxWidth: '260px' }} />
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: THEME.textLow }}>
          <span className="material-symbols-rounded" style={{ fontSize: '48px', display: 'block', marginBottom: '12px', color: THEME.outline }}>local_shipping</span>
          <div style={{ fontSize: '14px' }}>No hired vehicles found</div>
        </div>
      ) : (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            <thead>
              <tr style={{ background: THEME.surfaceVar }}>
                {['Contractor', 'Description', 'Rego', 'Type', 'Daily Rate', 'Days Active', 'Est. Cost', 'Status', ''].map((h, i) => (
              ...
              {filtered.map(d => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 12px', color: THEME.textMed }}>{d.vehicle_type || '-'}</td>
            ...
            <div style={{ padding: '20px 24px', overflowY: 'auto', flex: 1 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
                ...
                <div style={fieldWrap}>
                  ...
                  <input style={inp} value={form.vehicle_type} onChange={e => set('vehicle_type', e.target.value)} />
                </div>
                <div style={fieldWrap}>
                ...
                <div style={fieldWrap}>
                  <label style={lbl}>KM Rate</label>
                  <input style={inp} type="number" step="0.01" value={form.km_rate} onChange={e => set('km_rate', e.target.value)} />
                </div>
                <div style={fieldWrap}>
                ...
                <div style={fieldWrap}>
                  <label style={lbl}>Assigned Operator</label>
                  <select style={inp} value={form.assigned_operator} onChange={e => set('assigned_operator', e.target.value)}>
                    <option value="">-- None --</option>
                ...
                <div style={{ ...fieldWrap, display: 'flex', alignItems: 'center', gap: '8px', marginTop: '22px' }}>
                  <input type="checkbox" checked={form.driver_included} onChange={e => set('driver_included', e.target.checked)}
                    style={{ width: '16px', height: '16px', accentColor: color }} />
                  <label style={{ fontSize: '13px', color: THEME.text, cursor: 'pointer' }} onClick={() => set('driver_included', !form.driver_included)}>Driver Included</label>
                </div>
                <div style={{ ...fieldWrap, display: 'flex', alignItems: 'center', gap: '8px', marginTop: '22px' }}>
