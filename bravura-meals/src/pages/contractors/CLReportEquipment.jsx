...
export default function CLReportEquipment({ setPage }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data, error } = await supabase
        ...
        .select('id, description, contractor_id, serial_number, daily_rate, status, start_date, end_date, is_archived')
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ fontSize: '18px', fontWeight: 700, color: THEME.text, marginBottom: '16px' }}>Equipment Hire Report</div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '12px', marginBottom: '16px' }}>
        <KpiCard label="Total Equipment" value={kpis.total} icon="construction" accent={color} />
        <KpiCard label="Active" value={kpis.active} icon="check_circle" accent={THEME.success} progress={activePct} sub={activePct !== null ? `${activePct.toFixed(0)}% of equipment` : undefined} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginBottom: '16px', alignItems: 'center' }}>
        ...
        <button onClick={() => exportCsv('equipment-hire-report.csv',
          ['Description', 'Contractor', 'Serial Number', 'Daily Rate', 'Days Active', 'Est. Cost', 'Status'],
          filtered.map(r => [r.description, r.contractorName, r.serial_number || '', r.daily_rate, r.daysActive, r.estimatedCost.toFixed(2), r.status])
      ...
      {kpis.total > 0 && (
        <DashCard style={{ marginBottom: '16px' }}>
          <SectionTitle title="Active Equipment" subtitle="Share of hired equipment currently active" />
          <DonutGauge
            pct={activePct}
            color={color}
            label="active"
      ...
      <DashCard style={{ padding: '12px 16px' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <th style={th}>Description</th><th style={th}>Contractor</th><th style={th}>Serial Number</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id || i}>
                  <td style={td}>{r.description}</td><td style={td}>{r.contractorName}</td><td style={td}>{r.serial_number || '-'}</td>
