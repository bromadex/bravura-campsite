    ...
    try {
      ...
      setResultMeta({
        ...
        assetLabel:   form.asset_type === 'vehicle'
          ? (selVehicle ? `${selVehicle.registration || selVehicle.fleet_number || selVehicle.asset_number || '—'}${selVehicle.description ? ' · ' + selVehicle.description : ''}` : null)
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid black', marginBottom: '12px' }}>
            ...
            <tbody>
              {br.rows.map((r, i) => {
                ...
                const label = v ? `${v.registration || v.fleet_number || v.asset_number || '—'}${v.description ? ' · ' + v.description : ''}` : eq ? (eq.description || eq.fleet_number || eq.asset_number) : '—'
                return (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {br.rows.map((r, i) => {
                ...
                const label = v ? `${v.registration || v.fleet_number || v.asset_number || '—'}${v.description ? ' · ' + v.description : ''}` : eq ? eqLabel(eq) : '—'
                return (
                        ...
                        {row.asset_type === 'vehicle' ? (
                          <select value={row.vehicle_id} onChange={e => updateBulkRow(idx, 'vehicle_id', e.target.value)}
                            ...
                            {activeVehicles.map(v => (
                              <option key={v.id} value={v.id}>{v.registration || v.fleet_number || v.asset_number || '—'}{v.description ? ' · ' + v.description : ''}</option>
                            ))}
                          </select>
                        ) : (
            ...
            {form.asset_type === 'vehicle' && (
              <>
                <SearchSelect
                  ...
                  renderItem={v => `${v.registration || v.fleet_number || v.asset_number || '—'}${v.description ? ' · ' + v.description : ''}${v.fuel_types?.name ? ' · ' + v.fuel_types.name : ''}`}
                  renderSelected={v => `${v.registration || v.fleet_number || v.asset_number || '—'}${v.description ? ' · ' + v.description : ''}`}
                />
